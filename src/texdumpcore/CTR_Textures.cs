...
namespace CTR
{
    ...
    public class TextureUtil
    {
        ...
        public static Image<Bgra32> DecodeBCLIM(string filename)
        {
        ...
        public static Image<Bgra32> DecodeTexture(CTRTexture tex)
        {
        ...
        private static Image<Bgra32> GetGenericBitmap(CTRTexture tex)
        {
            ...
            int bytes = size.Width * size.Height * 4; // 4 bytes per pixel for rgba8888.
            ...
            for (int y = 0; y < size.Height; y++)
            {
                for (int x = 0; x < size.Width; x++)
                {
                    ...
                    if (tex.LuminanceBitSize > 0)
                    {
                        if (tex.Format == TextureFormat.la8 || tex.Format == TextureFormat.la4)
                        {
                            var a = Alpha; Alpha = Luminance; Luminance = a;
                        }
            ...
            var image = Image.LoadPixelData<Bgra32>(rgbaValues, size.Width, size.Height);

            if (crop)
            {
                image.Mutate(ctx => ctx.Crop(tex.Size.Width, tex.Size.Height));
            }

            return image;
        }

        private static Image<Bgra32> GetEtcBitmap(CTRTexture tex)
        {
            int bytes = tex.Size.Width * tex.Size.Height * 4; // 4 bytes per pixel for rgba8888.
            byte[] rgbaValues = new byte[bytes];
            ...
            var image = Image.LoadPixelData<Bgra32>(rgbaValues, tex.Size.Width, tex.Size.Height);
